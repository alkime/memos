...
These onboarding sessions are expensive, but well worth it. A fully onboarded teammate is highly functioning and independent, greatly increasing what the team as a whole can achieve. It's truly awesome and probably the thing I love most about being a team—like Voltron coming together, the whole is greater than the sum of its parts.
...
As I've onboarded (yuk yuk) to Claude Code, this analogy—as overwrought as it may be—has been for me a slightly deeper insight than all the pro-tip sorts of posts you see on r/claudeai (don't get me wrong, I love me a good "how I finally got Claude to hack the planet!" post).

## The Importance of Learning

This is all to say that Memory[^1] is important. It's no surprise that the first thing you do when you initialize Claude Code[^2] is build its initial Memory (`/init`). It's also important to build in processes that curate and expand this ground truth—we want our "onboarding docs" to be as high quality and useful as possible, allowing our LLM to get to a higher functioning state without a lot of in-session prompting.

This is the core reason why I'm a strong proponent of building in ways for Claude to [access code reviews]({{< ref "posts/2025-11-pr-comments-as-training-loop.md" >}}). When I review the LLM's code, I'm giving it feedback that can be captured into its Memory so there's a higher likelihood (though not 100%, of course!) that it won't make that mistake again. We could call this a form of learning.

Look for places to introduce learning loops into your LLM workflows. It's not just on code changes that I try to close Memory gaps. I've put together a [custom slash command](https://github.com/alkime/memos/blob/main/.claude/commands/docs/update-architecture.md) that is in a similar vein to what I do with addressing PR comments, but meant to fill gaps in the architecture docs portion of Claude's Memory. I'll do it less frequently and more outside the normal change merging cadence, but it's an important way to help keep the onboarding docs fresh.

---

#### Footnotes

[^1]: See https://code.claude.com/docs/en/memory for more on Claude Code Memory. It's more than just `CLAUDE.md`, and here when I reference Memory it's _all_ of the context loaded up at session startup.
[^2]: I've been using Claude Code, so I use the parlance of that platform. My sense is that the lexicon is transferable, sometimes directly, but at the least probably can be swapped for others between the SOTA coding assistants. Memory would be an example of direct terms, whereas maybe "planning mode" might mean something different for Claude Code than it does for Codex, etc.
